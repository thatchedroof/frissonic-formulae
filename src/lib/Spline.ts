/**
 * From typescript-cubic-spline (https://github.com/starnutoditopo/typescript-cubic-spline)
 * Original work Copyright (c) 2021 Starnuto di topo
...
class Spline {
  ...
  toJSON() {
    return {
      xs: this.xs,
      ys: this.ys,
      ks: Array.from(this.ks),
    }
  }

  static fromJSON(json: { xs: number[]; ys: number[]; ks: number[] }) {
    const spline = new Spline(json.xs, json.ys)
    spline.ks = Float64Array.from(json.ks)
    return spline
  }
